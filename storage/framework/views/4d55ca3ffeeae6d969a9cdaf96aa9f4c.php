...
$sectionLabel = function(string $label) use ($isMobile) {
    if ($isMobile) {
        return "<div class=\"px-3 pb-1 pt-3\"><span class=\"text-[10px] font-black uppercase tracking-widest text-slate-400\">{$label}</span></div>";
    }
    return <<<HTML
<div class="px-3 pb-1 pt-3 transition-opacity duration-300" x-bind:class="sidebarCollapsed ? 'text-center' : ''">
    <span x-show="!sidebarCollapsed" class="text-[10px] font-black uppercase tracking-widest text-slate-400">{$label}</span>
    <span x-show="sidebarCollapsed" class="text-xs font-black uppercase tracking-widest text-slate-300" title="{$label}">···</span>
</div>
HTML;
};
?>
...
<?php echo $navLink(route('dashboard'), 'Dashboard',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($user?->role, ['admin', 'teacher', 'bursar'])): ?>
    <?php echo $sectionLabel('People'); ?>
...
<?php echo $navLink(route('students.index'), 'Students',
...
<?php else: ?>
<?php echo $navLink(route('students.index'), 'My Children',
...
<?php echo $navLink(route('teachers'), 'Teachers',
    'bg-orange-100', 'text-orange-500',
    '<path stroke-linecap="round" stroke-linejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>',
    request()->routeIs('teachers') || request()->routeIs('teachers.*')); ?>


<?php echo $navLink(route('parents.index'), 'Parents',
    'bg-pink-100', 'text-pink-500',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($user?->role, ['admin', 'teacher'])): ?>
    <?php echo $sectionLabel('Academics'); ?>


    <?php echo $navLink(route('classes.index'), 'Classes',
        'bg-slate-100', 'text-slate-500',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>',
        request()->routeIs('classes.*')); ?>


    <?php echo $navLink(route('subjects.index'), 'Subjects',
        'bg-indigo-100', 'text-indigo-500',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>',
        request()->routeIs('subjects.*')); ?>


    <?php echo $navLink(route('results.entry'), 'Score Entry',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user?->role === 'admin'): ?>
    <?php echo $navLink(route('results.broadsheet'), 'Broadsheet',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user?->role === 'admin' || ($user?->role === 'teacher' && $user?->is_class_teacher)): ?>
    <?php echo $navLink(route('attendance'), 'Attendance',
        'bg-teal-100', 'text-teal-500',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>',
        request()->routeIs('attendance')); ?>


    <?php echo $navLink(route('homework.index'), 'Homework',
        'bg-purple-100', 'text-purple-500',
        '<path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>',
        request()->routeIs('homework.*')); ?>


    <?php echo $navLink(route('messages'), 'Messages',
    ...
    <?php echo $navLink($cbtHref, 'CBT Exams',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user?->role === 'bursar'): ?>
    <?php echo $sectionLabel('Finance'); ?>

    <?php echo $navLink(route('billing.index'), 'Billing',
        'bg-emerald-100', 'text-emerald-500',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($user?->role, ['admin', 'teacher', 'bursar'])): ?>
    <?php echo $sectionLabel('System'); ?>

    <?php echo $navLink(route('more-features'), 'More Features',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user?->role === 'admin'): ?>
    <?php echo $navLink(route('settings.index'), 'Settings',
    ...
    <?php echo $navLink(route('settings.subscription'), 'Billing',
        'bg-green-100', 'text-green-500',
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user?->is_super_admin): ?>
    <?php echo $navLink(route('superadmin.dashboard'), 'Dev Dashboard',
...
<?php echo $navLink(route('profile'), 'My Profile',
    'bg-violet-100', 'text-violet-500',
    '<path stroke-linecap="round" stroke-linejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>',
    request()->routeIs('profile')); ?>


<?php /**PATH C:\laragon\www\myacademy-laravel\resources\views/layouts/partials/app-nav.blade.php ENDPATH**/ ?>