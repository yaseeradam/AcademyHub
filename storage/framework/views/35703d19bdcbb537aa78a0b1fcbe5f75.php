    ...
    <div class="fixed bottom-24 right-6 z-50 w-80 md:w-96 bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-200"
        ...
        <div class="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col space-y-4" style="flex: 1; padding: 1rem; overflow-y: auto; background-color: #f9fafb; display: flex; flex-direction: column; gap: 1rem; height: 350px;">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex w-full <?php echo e($msg['role'] == 'user' ? 'justify-end' : 'justify-start'); ?>"
                     style="display: flex; width: 100%; justify-content: <?php echo e($msg['role'] == 'user' ? 'flex-end' : 'flex-start'); ?>;">
                     
                    <div class="max-w-[80%] rounded-lg p-3 text-sm shadow-sm <?php echo e($msg['role'] == 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-gray-100'); ?>"
                         style="max-width: 80%; padding: 0.75rem; border-radius: 0.5rem; font-size: 0.875rem; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); word-wrap: break-word; white-space: pre-wrap; font-family: sans-serif; <?php echo e($msg['role'] == 'user' ? 'background-color: #2563eb; color: white; border-bottom-right-radius: 0;' : 'background-color: white; color: #1f2937; border-bottom-left-radius: 0; border: 1px solid #f3f4f6;'); ?>">
...
<?php /**PATH C:\laragon\www\myacademy-laravel\resources\views/livewire/agent-pro-chat.blade.php ENDPATH**/ ?>