...
class _AdminHomeState extends State<AdminHome> with SingleTickerProviderStateMixin {
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          children: [
            _statCard('Students', '${_students.length}', Icons.people, const Color(0xFF3B82F6), onTap: () => context.push('/students')),
        ...
        GridView.count(
          ...
          children: [
            _actionCard('Attendance', Icons.how_to_reg, const Color(0xFF10B981), () => context.push('/attendance')),
            _actionCard('Scores', Icons.edit_note, const Color(0xFF3B82F6), () => context.push('/scores')),
            _actionCard('Homework', Icons.assignment, const Color(0xFF8B5CF6), () => context.push('/homework')),
          ],
        ),
        const SizedBox(height: 20),
  ...
  Widget _statCard(String label, String value, IconData icon, Color color, {bool pendingStream = false, VoidCallback? onTap}) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.all(14),
    ...
    return RefreshIndicator(
      ...
      child: _homework.isEmpty
          ? ListView(
              ...
              children: const [
                SizedBox(height: 100),
                Center(child: Text('No homework yet.', style: TextStyle(color: Color(0xFF64748B)))),
              ],
            )
          : ListView.separated(
              physics: const AlwaysScrollableScrollPhysics(),
          ...
          child: SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                showDialog(
                  context: context,
                  builder: (context) => const AnnouncementCreateDialog(),
                ).then((val) {
                  if (val == true) _load();
                });
              },
              icon: const Icon(Icons.add, color: Colors.white),
              label: const Text('Publish Announcement', style: TextStyle(color: Colors.white)),
          ...
          child: RefreshIndicator(
            ...
            child: _announcements.isEmpty
                ? ListView(
                    ...
                    children: const [
                      SizedBox(height: 100),
                      Center(child: Text('No announcements.', style: TextStyle(color: Color(0xFF64748B)))),
                    ],
                  )
                : ListView.separated(
                    physics: const AlwaysScrollableScrollPhysics(),
