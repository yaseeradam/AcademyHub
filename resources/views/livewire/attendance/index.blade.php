<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl shadow-xl" style="background-color: #1a2e4a;">
        ...
        <div class="relative flex flex-col gap-4 px-8 py-8 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1.5 text-base font-medium" style="color:#93c5fd;">{{ now()->format('l, F j, Y') }}</p>
            </div>
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            <div class="mt-4 text-3xl font-black text-slate-900">{{ $dayCounts['Present'] ?? 0 }}</div>
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            <div class="mt-4 text-3xl font-black text-slate-900">{{ $dayCounts['Absent'] ?? 0 }}</div>
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            <div class="mt-4 text-3xl font-black text-slate-900">{{ $dayCounts['Late'] ?? 0 }}</div>
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            <div class="mt-4 text-3xl font-black text-slate-900">{{ $dayCounts['Excused'] ?? 0 }}</div>
    ...
    <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <input wire:change="$set('date', $event.target.value)" type="date" value="{{ $date }}" class="input-compact" />
            <button wire:click="$set('date', '{{ now()->toDateString() }}')" class="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-colors">Today</button>
        </div>
    </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div class="text-sm font-bold text-slate-800">
                {{ $date ? \Carbon\Carbon::parse($date)->format('l, F j, Y') : 'Today' }}
            </div>
            <span class="text-xs font-semibold text-slate-400">{{ $dateRecords->count() }} records</span>
        </div>
            ...
            <div class="overflow-x-auto">
                <x-table>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @foreach($dateRecords as $mark)
                            @php $student = $mark->student; @endphp
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-3">
                                        @if($student?->passport_photo_url)
                                            <img src="{{ $student->passport_photo_url }}" class="h-9 w-9 rounded-full object-cover ring-2 ring-slate-100 flex-shrink-0"/>
                                        @else
                                ...
                                <td class="px-4 py-3.5 text-sm text-slate-600">{{ $student?->schoolClass?->name ?? 'N/A' }}</td>
                                <td class="px-4 py-3.5">
                                    <x-status-badge variant="{{ $mark->status === 'Present' ? 'success' : ($mark->status === 'Absent' ? 'danger' : ($mark->status === 'Late' ? 'warning' : 'default')) }}">
                                        {{ $mark->status }}
                                    </x-status-badge>
                                </td>
                                <td class="px-4 py-3.5 text-sm text-slate-500">{{ $mark->note ?: '—' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </x-table>
        ...
        @else
            <div class="px-6 py-14 text-center">
                ...
                <div class="text-sm font-bold text-slate-700">No attendance for {{ $date ? \Carbon\Carbon::parse($date)->format('F j, Y') : 'today' }}</div>
            ...
            <div class="absolute inset-0 sm:inset-auto sm:right-0 sm:top-0 sm:bottom-0 sm:w-[480px] bg-white flex flex-col shadow-2xl">
                ...
                <div class="flex-shrink-0 px-5 py-4 border-b border-slate-100" style="background-color:#1a2e4a;">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <div class="text-xs mt-0.5" style="color:#93c5fd;">{{ $date ? \Carbon\Carbon::parse($date)->format('l, M j') : 'Today' }}</div>
                        </div>
                        <div class="flex items-center gap-2">
                            @if($classId && $sectionId)
                                <span class="rounded-full bg-emerald-500 px-2.5 py-0.5 text-[11px] font-bold text-white">{{ $markCounts['Present'] ?? 0 }} P</span>
                                <span class="rounded-full bg-red-500 px-2.5 py-0.5 text-[11px] font-bold text-white">{{ $markCounts['Absent'] ?? 0 }} A</span>
                            @endif
                ...
                <div class="flex-shrink-0 grid grid-cols-2 gap-3 border-b border-slate-100 bg-slate-50 px-5 py-4">
                    ...
                    <div>
                        ...
                        <select wire:model.live="sectionId" class="select w-full" @disabled(!$classId)>
                    ...
                    @if($classId && $sectionId)
                        <div class="space-y-2">
                            @foreach($visibleStudents as $student)
                                @php $status = $marks[$student->id]['status'] ?? 'Present'; @endphp
                                <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                                    <div class="flex items-center gap-3 mb-3">
                                        @if($student->passport_photo_url)
                                            <img src="{{ $student->passport_photo_url }}" class="h-10 w-10 flex-shrink-0 rounded-full object-cover ring-2 ring-slate-100"/>
                                        @else
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="truncate text-sm font-bold text-slate-900">{{ $student->full_name }}</div>
                                            <div class="text-xs text-slate-400">{{ $student->admission_number }}</div>
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-4 gap-1.5">
                                        <button wire:click="setMark({{ $student->id }}, 'Present')"
                                                class="rounded-xl py-2 text-xs font-bold transition-all {{ $status === 'Present' ? 'bg-emerald-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                                            Present
                                        </button>
                                        <button wire:click="setMark({{ $student->id }}, 'Absent')"
                                                class="rounded-xl py-2 text-xs font-bold transition-all {{ $status === 'Absent' ? 'bg-red-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                                            Absent
                                        </button>
                                        <button wire:click="setMark({{ $student->id }}, 'Late')"
                                                class="rounded-xl py-2 text-xs font-bold transition-all {{ $status === 'Late' ? 'bg-amber-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                                            Late
                                        </button>
                                        <button wire:click="setMark({{ $student->id }}, 'Excused')"
                                                class="rounded-xl py-2 text-xs font-bold transition-all {{ $status === 'Excused' ? 'bg-violet-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                                            Excused
                                        </button>
                                    </div>
                                </div>
