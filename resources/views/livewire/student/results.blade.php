                ...
                <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach($scores as $score)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-900">{{ $score->subject->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm text-gray-900">{{ $score->ca1 }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm text-gray-900">{{ $score->ca2 }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm text-gray-900">{{ $score->exam }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                        @if($score->grade === 'A') bg-green-100 text-green-800
                                        @elseif($score->grade === 'B') bg-blue-100 text-blue-800
                                        @elseif($score->grade === 'C') bg-yellow-100 text-yellow-800
                                        @elseif($score->grade === 'D') bg-orange-100 text-orange-800
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-center text-sm font-bold text-gray-900">{{ $scores->sum('ca1') }}</td>
                            <td class="px-6 py-4 text-center text-sm font-bold text-gray-900">{{ $scores->sum('ca2') }}</td>
                            <td class="px-6 py-4 text-center text-sm font-bold text-gray-900">{{ $scores->sum('exam') }}</td>
                            <td class="px-6 py-4 text-center text-sm font-bold text-gray-900">{{ $scores->sum('total') }}</td>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="rounded-2xl bg-white p-6 shadow-lg">
                <div class="text-center">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $scores->count() }}</p>
                </div>
            </div>
            <div class="rounded-2xl bg-white p-6 shadow-lg">
                <div class="text-center">
                    <p class="text-sm font-semibold text-gray-600">Average Score</p>
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ round($scores->avg('total'), 1) }}%</p>
                </div>
            </div>
            <div class="rounded-2xl bg-white p-6 shadow-lg">
                <div class="text-center">
                    <p class="text-sm font-semibold text-gray-600">Highest Score</p>
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $scores->max('total') }}</p>
                    <p class="text-xs text-gray-500 mt-1">{{ $scores->where('total', $scores->max('total'))->first()->subject->name ?? '' }}</p>
                </div>
            </div>
        </div>
    @endif
