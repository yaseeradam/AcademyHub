            ...
            <div id="manage-{{ $tenant->id }}" style="display:none;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        ...
                        <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:8px;overflow:hidden;">
                            <button type="button"
                                    onclick="toggleResetPw({{ $tenant->id }})"
                                    style="width:100%;padding:10px 12px;background:transparent;border:none;color:#1d4ed8;font-size:13px;font-weight:700;cursor:pointer;text-align:left;display:flex;align-items:center;justify-content:space-between;gap:8px;">
                                ...
                                <svg id="resetPwChevron-{{ $tenant->id }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" style="width:13px;height:13px;transition:transform .2s;">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </button>
                            <div id="resetPwForm-{{ $tenant->id }}" style="display:none;padding:12px;border-top:1px solid #e2e8f0;">
                                <form id="rpw-{{ $tenant->id }}" action="{{ route('superadmin.tenants.reset-admin-password', $tenant) }}" method="POST">
                                    @csrf
                                    <div style="margin-bottom:8px;">
                                        <label style="font-size:11px;font-weight:700;color:#475569;display:block;margin-bottom:4px;">New Password</label>
                                    ...
                                    <div style="margin-bottom:10px;">
                                        <label style="font-size:11px;font-weight:700;color:#475569;display:block;margin-bottom:4px;">Confirm Password</label>
                                        <input type="password" name="new_password_confirmation" required minlength="8"
                                               placeholder="Repeat password"
                                               style="width:100%;padding:8px 10px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
                                    </div>
                                    <button type="button"
                                            onclick="saConfirm('rpw-{{ $tenant->id }}', 'Reset admin password for {{ addslashes($tenant->name) }}?')"
                                            style="width:100%;padding:9px;background:#1d4ed8;border:none;border-radius:8px;color:#fff;font-size:13px;font-weight:700;cursor:pointer;">
                                        Set New Password
                                    </button>
                                </form>
                            </div>
...
function toggleResetPw(id) {
    const form = document.getElementById('resetPwForm-' + id);
    const chevron = document.getElementById('resetPwChevron-' + id);
