    ...
    private function processIncomingWebhookMessage(string $phone, string $text, ?string $buttonId = null): void
    {
        ...
        if ($buttonId && str_starts_with($buttonId, 'rc_std_')) {
            ...
            if ($studentObj) {
                ...
                // Verify if broadsheet results are officially published for this student's class, term, and session
                $studentClassIds = \App\Models\Score::where('student_id', $studentObj->id)
                    ->where('term', $term)
                    ->where('session', $session)
                    ->pluck('class_id')
                    ->unique()
                    ->filter()
                    ->push($studentObj->class_id)
                    ->toArray();
                ...
                if (!$isPublished) {
                    $this->sendMetaMessage($phone, "⚠️ *Report Card Unavailable*\n\nThe official report card for Term {$term} ({$session}) has not been published yet by the school administration.");
                    return;
                }
        ...
        $prompt = "You are HubGenie, the official school coordinator assistant for the AcademyHub school management system.\n" .
                  ...
                  "9. WHATSAPP FORMATTING RULES:\n" .
                  "   - BOLD: Format bold text using single asterisks (e.g., *this is bold*). NEVER use double asterisks (**).\n" .
                  "   - VERTICAL BULLET LISTS: ALWAYS format scores, attendance records, homework, fees, and schedules as clean, structured vertical lists. NEVER pack multiple scores or metrics horizontally on a single line with pipes. Place EACH metric on its own separate line starting with a bullet `• `.\n" .
                  "   - HEADINGS: Use bold text with a clear emoji for section headers (e.g., 📊 *ACADEMIC RESULTS*, 📅 *ATTENDANCE STATUS*, 💰 *TUITION FEES*).\n" .
                  "   - SCORE FORMATTING LAYOUT: Format subject scores vertically as a clean block per subject:\n" .
                  "     📘 *Mathematics*\n" .
                  "     • CA 1: 15/20\n" .
                  "     • CA 2: 15/20\n" .
                  "     • Exam: 50/60\n" .
                  "     • *Total Score*: *80/100*\n" .
                  "     • *Grade*: A\n" .
                  "   - LINE BREAKS: Use clear single line breaks between list items and double line breaks between subject blocks to make the message effortless and fast to read on any phone screen.\n" .
                  "10. DYNAMIC REPORT CARDS (PDF Delivery):\n" .
                  ...
                  "   - You can output multiple SEND_PDF tags if they request report cards for multiple children or multiple terms in a single prompt.\n" .
                  "11. INTENT DETECTION & SUPPORT TICKETS: If the user indicates they want to report an issue, log a complaint, offer feedback, report missing grades/attendance, or request a call back from the school administration, you MUST append a hidden tag at the very end of your response: '[SUPPORT_TICKET_DETECTED: <message>]' where <message> is a clear, concise 1-sentence summary of the user's actual problem or concern. Do not include this tag for standard information questions (e.g. asking for grades, schedules, or events).\n" .
                  "12. HANDLING STUDENT RESULTS & SCORES: When asked about student scores, academic performance, grades, exam results, or a SPECIFIC SUBJECT score (e.g. 'What did Abdullahi get in Maths?', 'Show me his English score'): Check if the child's name or admission number is specified in the question. If not, and there are multiple children listed in the context, ask the user to specify which student. Once identified, look up the requested subject (or all subjects if general) in `recent_scores`. Format each subject as a clean vertical bulleted block (CA 1 on line 1, CA 2 on line 2, Exam on line 3, Total Score & Grade on line 4 & 5). NEVER pack CA1, CA2, Exam, and Total horizontally on a single line. However, if the user explicitly asks to download or get an official PDF Report Card, check if a matching term and session exists in `published_results`. If `published_results` does NOT contain that term/session, inform them naturally that the official PDF Report Card has not been published yet by the school administration and do NOT generate any '[SEND_PDF]' or '[AMBIGUOUS_REPORT_CARD]' tags.\n" .
                  "13. ACTIVE SESSION STUDENT: If 'active_session_student_id' is set in the context (not null), this indicates the student the parent has been actively chatting about or selected recently. Prioritize this student in your answers unless the user names a different child. Additionally, if your answer resolves to or discusses a specific single student from the context, you MUST append a hidden tag at the very end of your response: '[ACTIVE_STUDENT_SELECTED: <student_id>]' where <student_id> is that student's ID from the context. Do not append this tag if you are talking about multiple children or general school information.\n" .
